...
<form id="add-class" action="{{ route('game.item_edit', [ 'item_id' => $item->id ]) }}" method="POST" enctype="multipart/form-data">
    ...
    <p><div class="p-3">
        <label for="passive" class="inline-flex relative items-center cursor-pointer">
            <input type="checkbox" id="passive" name="passive" value="yes" class="sr-only peer"{{ $item->passive ? ' checked' : '' }}>
    ...
    <p><div class="p-3">
        ...
        <select id="inspire" name="inspire" class="form-select w-48 m-0 px-3 py-2 text-base font-normal transition ease-in-out rounded border border-gray-300 dark:border-gray-400 bg-white dark:bg-gray-700 text-black dark:text-gray-200">
            <option value="">無</option>
            <option value="protect"{{ $item->inspire == 'protect' ? ' selected' :''}}>護衛</option>
            <option value="apportion"{{ $item->inspire == 'apportion' ? ' selected' :''}}>分散傷害</option>
            <option value="reflex"{{ $item->inspire == 'reflex' ? ' selected' :''}}>傷害反射</option>
            <option value="hatred"{{ $item->inspire == 'hatred' ? ' selected' :''}}>集中仇恨</option>
            <option value="invincible"{{ $item->inspire == 'invincible' ? ' selected' :''}}>無敵狀態</option>
            <option value="throw"{{ $item->inspire == 'throw' ? ' selected' :''}}>投擲道具</option>
    ...
    <p><div class="p-3">
        <label for="times" class="text-base">持續時間：
            <input id="times" class="w-16 rounded border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700"
                type="number" name="effect_times" min="0" max="480" step="10" value="{{ $item->effect_times }}">
