...
<form id="add-class" action="{{ route('game.monster_edit', [ 'monster_id' => $monster->id ]) }}" method="POST">
    ...
    <p><div class="p-3">
        ...
        <div class="inline">
            ...
            <input id="base_dp" class="w-16 rounded border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700"
                type="number" name="dp" min="10" max="50" step="1" value="{{ $monster->dp }}">
    ...
    <p><div class="p-3">
        <button data-modal-toggle="colorModal" type="button" id="picker" class="{{ $monster->style }} bg-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
            難易度顏色碼
        </button>
    </div></p> 
    <p class="p-6">
...
<div id="colorModal" data-modal-target="colorModal" data-modal-placement="center-center" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-[70] hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
