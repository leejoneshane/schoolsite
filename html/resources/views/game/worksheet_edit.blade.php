...
<form id="add-class" action="{{ route('game.worksheet_edit', [ 'worksheet_id' => $worksheet->id ]) }}" method="POST">
    @csrf
    <input type="hidden" name="uuid" value="{{ $teacher->uuid }}">
    <p><div class="p-3">
        <label for="title" class="text-base">學習單標題：</label>
        <input type="text" id="title" name="title" value="{{ $worksheet->title }}" class="inline w-48 rounded border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700 text-black dark:text-gray-200" required>
    </div></p>
    <p><div class="p-3">
        ...
        <select id="subject" name="subject" class="inline w-48 rounded border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700 text-black dark:text-gray-200">
            ...
            <option value="{{ $sub->name }}"{{ $sub->name == $worksheet->subject ? ' selected' : ''}}>{{ $sub->name }}</option>
            @endforeach
        </select>
    </div></p>
    <p><div class="p-3">
        <label for="desc" class="text-base">學習目標：</label>
        <input type="text" id="desc" name="desc" value="{{ $worksheet->description }}" class="inline w-1/2 rounded border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700 text-black dark:text-gray-200" required>
    </div></p>
    <p><div class="p-3">
        ...
        <select id="grade" name="grade" class="inline w-128 rounded border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700 text-black dark:text-gray-200">
            @foreach ($grades as $g)
            <option value="{{ $g->id }}"{{ $g->id == $worksheet->grade_id ? ' selected' : '' }}>{{ $g->name }}</option>
            @endforeach
        </select>
    </div></p>
    <p><div class="p-3">
        <label for="editor" class="text-base">故事綱要：</label>
        <textarea id="editor" name="intro" rows="8" required>{{ $worksheet->intro }}</textarea>
    </div></p>
    <p><div class="p-3">
        ...
        <ul class="grid w-full gap-6 md:grid-cols-5">
            @foreach ($maps as $m)
            <li>
                <input type="radio" id="{{ $m->id }}" name="map" value="{{ $m->id }}" class="hidden peer" {{ $m->id == $worksheet->map_id ? ' checked' : '' }}/>
    ...
    <p><div class="p-3">
        <label for="share" class="inline-flex relative items-center cursor-pointer">
            <input type="checkbox" id="share" name="share" value="yes" class="sr-only peer"{{ $worksheet->share ? ' checked' : '' }}>
