...
<div class="text-2xl font-bold leading-normal pb-5 drop-shadow-md">
    據點一覽表
    <a class="text-sm py-2 pl-6 rounded text-blue-500 hover:text-blue-600" href="{{ route('game.base_add') }}">
        <i class="fa-solid fa-circle-plus"></i>新增據點
    </a>
</div>
    ...
    <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-gray-700 dark:even:bg-gray-600">
        ...
        <td class="p-2">
            <a class="py-2 pr-6 text-blue-500 hover:text-blue-600"
                href="{{ route('game.base_edit', ['base_id' => $sk->id]) }}" title="編輯">
                <i class="fa-solid fa-pen"></i>
            </a>
            <button class="py-2 pr-6 text-red-300 hover:text-red-600" title="刪除"
                onclick="
                    const myform = document.getElementById('remove');
                    myform.action = '{{ route('game.base_remove', ['base_id' => $sk->id]) }}';
                    myform.submit();
            ">
