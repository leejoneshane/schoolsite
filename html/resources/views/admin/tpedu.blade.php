...
<form id="edit-unit" action="{{ route('sync') }}" method="POST">
    ...
    <br>
    <label for="sync_students" class="inline-flex relative items-center cursor-pointer">
        <input type="checkbox" id="sync_students" name="sync_students" value="yes" class="sr-only peer"
            onclick="
                if (this.checked) {
            ...
            ">
        <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
        <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">重新同步學生帳號！</span>
    </label>
    <br>
    <div id="target" class="p-2 hidden">
        <label for="target" class="text-sm font-medium">選擇同步對象：</label>
        <select class="inline rounded w-40 px-3 py-2 border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700 text-black dark:text-gray-200 text-sm font-medium"
            name="target">
