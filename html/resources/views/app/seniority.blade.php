...
<div class="text-2xl font-bold leading-normal pb-5">
    ...
    <a class="text-sm py-2 pl-6 rounded text-blue-300 hover:text-blue-600" href="{{ route('seniority.export') }}">
...
<table class="w-full py-4 text-left font-normal">
    ...
    <tr class="bg-gray-300 dark:bg-gray-500 font-semibold text-lg">
        <th scope="col" class="w-16 text-center bg-green-300 dark:bg-green-500">
            在校年
        </th>
        <th scope="col" class="w-16 text-center bg-green-300 dark:bg-green-500">
            在校月
        ...
        <th scope="col" class="w-16 text-center bg-blue-300 dark:bg-blue-500">
            在校年
        </th>
        <th scope="col" class="w-16 text-center bg-blue-300 dark:bg-blue-500">
            在校月
...
@else
    @foreach ($teachers as $seniority)
    <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-gray-700 dark:even:bg-gray-600">
        <td class="p-2">{{ ($seniority->teacher->tutor) ?: $seniority->teacher->role_name }}</td>
        <td class="p-2">{{ $seniority->teacher->realname }}</td>
        <td class="bg-green-300 dark:bg-green-500">{{ $seniority->school_year }}</td>
        <td class="bg-green-300 dark:bg-green-500">{{ $seniority->school_month }}</td>
        <td class="bg-green-300 dark:bg-green-500">{{ $seniority->teach_year }}</td>
        <td class="bg-green-300 dark:bg-green-500">{{ $seniority->teach_month }}</td>
        <td class="bg-green-300 dark:bg-green-500">{{ $seniority->years }}</td>
        <td class="bg-green-300 dark:bg-green-500">{{ $seniority->score }}</td>
        <td class="bg-blue-300 dark:bg-blue-500">{{ $seniority->new_school_year }}</td>
        <td class="bg-blue-300 dark:bg-blue-500">{{ $seniority->new_school_month }}</td>
        <td class="bg-blue-300 dark:bg-blue-500">{{ $seniority->new_teach_year }}</td>
        <td class="bg-blue-300 dark:bg-blue-500">{{ $seniority->new_teach_month }}</td>
        <td class="bg-blue-300 dark:bg-blue-500">{{ $seniority->newyears }}</td>
        <td class="bg-blue-300 dark:bg-blue-500">{{ $seniority->newscore }}</td>
        <td class="p-2">
            @if ($current == $year && (Auth::user()->is_admin ||  Auth::user()->uuid == $teacher->uuid))
