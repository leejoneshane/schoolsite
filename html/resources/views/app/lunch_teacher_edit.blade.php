...
@section('content')
    ...
    @if (empty($settings) || time() < strtotime($settings->survey_at))
        <div class="flex justify-center mt-6">
            <div class="w-full border-red-500 bg-red-100 dark:bg-red-700 border-b-2 mb-5 p-4" role="alert">
    ...
    @elseif (time() > strtotime($settings->expired_at))
        ...
        {{-- Display current data as read-only if desired, or just hide form --}}
        @if($teacher)
             <div class="bg-white rounded-lg shadow p-6 dark:bg-gray-800 mt-4">
                <h3 class="text-lg font-bold mb-4">您的登記資料：</h3>
                <p>身份：{{ $teacher->tutor ? '導師（隨班用餐）' : '一般教師' }}</p>
                <p>飲食偏好：
                    {{ $teacher->vegen ? '素食' : '葷食' }}，
                    {{ $teacher->milk ? '飲用牛奶' : '豆奶/不飲用牛奶' }}
                </p>
                {{-- Simplified view of schedule --}}
             </div>
        @endif
