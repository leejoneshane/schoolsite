...
@section('content')
    <div class="text-2xl font-bold leading-normal pb-5">
        ...
        @if ($manager)
            ...
            <a class="text-sm py-2 pl-6 rounded text-blue-300 hover:text-blue-600"
                href="{{ route('lunch.download', ['section' => $section, 'class_id' => $classroom->id]) }}">
            ...
            @if ($manager)
                <div>
                    ...
                    <select id="classes"
                        ...
                        onchange="
                        var cls = this.value;
                        window.location.replace('{{ route('lunch', ['section' => $section]) }}' + '?class=' + cls);
                        ">
                        <option></option>
                    ...
                    @foreach ($surveys as $s)
                        <tr>
                            ...
                            <td class="border border-1 text-sm text-center">
                                {!! ($s->by_school && !($s->vegen)) ? '<i class="fa-solid fa-check"></i>' : ''  !!}</td>
                            <td class="border border-1 text-sm text-center">
                                {!! ($s->by_school && $s->vegen) ? '<i class="fa-solid fa-check"></i>' : ''  !!}</td>
                            <td class="border border-1 text-sm text-center">
                                {!! ($s->by_school && $s->milk) ? '<i class="fa-solid fa-check"></i>' : ''  !!}</td>
                            <td class="border border-1 text-sm text-center">
                                {!! ($s->by_school && !($s->milk)) ? '<i class="fa-solid fa-check"></i>' : ''  !!}</td>
                            <td class="border border-1 text-sm text-center">
                                {!! ($s->by_parent) ? '<i class="fa-solid fa-check"></i>' : ''  !!}</td>
                            <td class="border border-1 text-sm text-center">
                                {!! ($s->boxed_meal) ? '<i class="fa-solid fa-check"></i>' : ''  !!}</td>
                        </tr>
                    @endforeach
                </table>
            @endif
