...
<form id="add-club" action="{{ route('repair.editkind', ['kind' => $kind->id]) }}" method="POST">
    @csrf
    <p class="p-3">
        <label for="title" class="inline">名稱：</label>
        <input type="text" id="title" name="title" value="{{ $kind->name }}" class="inline w-64 rounded border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700 text-black dark:text-gray-200" required>
    </p>
    <p class="p-3">
        <label for="roles" class="inline">二級維修人員：</label>
        <div id="nassign">
        @foreach ($kind->managers as $user)
