...
<form method="POST" action="{{ route('organize.arrange') }}">
    ...
    <div class="w-full">
        ...
        ">
            ...
            <option value="second"{{ ($display == 'second') ? ' selected' : '' }}>第一志願落選作業</option>
            <option value="third"{{ ($display == 'third') ? ' selected' : '' }}>第二志願落選作業</option>
            @endif
            @if ($flow->onFinish())
            <option value="four"{{ ($display == 'four') ? ' selected' : '' }}>第三志願落選作業</option>
            <option value="five"{{ ($display == 'five') ? ' selected' : '' }}>第四志願落選作業</option>
            <option value="six"{{ ($display == 'six') ? ' selected' : '' }}>第五志願落選作業</option>
            @endif
...
<table class="w-full p-4 text-left font-normal">
    <tr class="bg-gray-300 dark:bg-gray-500 font-semibold text-lg">
        ...
        <th scope="col" class="p-2">
            意願選填結果（<span class="bg-white">　</span>為保留職缺，<span class="bg-red-200">　</span>為第一志願，<span class="bg-orange-200">　</span>為第二志願，<span class="bg-yellow-200">　</span>為第三志願，<span class="bg-green-200">　</span>為第四志願，<span class="bg-blue-200">　</span>為第五志願，<span class="bg-purple-200">　</span>為第六志願）
        </th>
    </tr>
    ...
    <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-gray-700 dark:even:bg-gray-600">
        ...
        @foreach ($v->reserved() as $t)
            <span class="pl-4 bg-white text-gray-500">{{ $t->realname }}</span>
        @endforeach
        ...
        @endforeach
        @if ($v->assigned < $v->$v->shortfall - $v->filled && $display == "seven")
        @for ($z=1;$z< $v->$v->shortfall - $v->filled - $v->assigned;$z++)
            <select id="t{{ $v->id }}_{{ $z }}">
                <option value="">未指派</option>
				@foreach ($rest_teachers as $t)
                <option value="{{ $t->uuid }}"{{ $teachers[$v->id]->has($t) ? ' selected' : '' }}>
                    {{ $t->realname }}
                </option>
				@endforeach
            </select>
    ...
    <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-gray-700 dark:even:bg-gray-600">
        ...
        @foreach ($teachers[$v->id] as $t)
            <span class="pl-4 bg-red-200">
                <input id='s{{ $v->id }}_{{ $t->uuid }}' type='checkbox'{{ (in_array($v->id, $t->special)) ? ' checked' : '' }}>
                <button id='{{ $t->uuid }}' data-modal-toggle="defaultModal">
                    {{ $t->teacher->realname }}
                </button>
            </span>
...
function arrange(event) {
    ...
    var vid = myid.substring(1, myid.indexOf('_') - 1);
