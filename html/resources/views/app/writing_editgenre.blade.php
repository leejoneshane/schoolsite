...
<form id="add-roster" action="{{ route('writing.editgenre', ['genre' => $genre->id]) }}" method="POST">
    @csrf
    <p class="p-3">
        <label for="title" class="inline">專欄名稱：</label>
        <input type="text" id="title" name="title" value="{{ $genre->name }}" class="inline w-64 rounded border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700 text-black dark:text-gray-200" required>
    ...
    <p class="p-3">
        ...
        <textarea id="desc" class="inline w-64 rounded border border-gray-300 focus:border-blue-700 focus:ring-1 focus:ring-blue-700 focus:outline-none active:outline-none dark:border-gray-400 dark:focus:border-blue-600 dark:focus:ring-blue-600  bg-white dark:bg-gray-700 text-black dark:text-gray-200"
            name="desc" rows="5" cols="120">{{ $genre->description }}</textarea>
    </p>
    <p class="p-6">
        <div class="inline">
