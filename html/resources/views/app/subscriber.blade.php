    ...
    <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-gray-700 dark:even:bg-gray-600">
        <td class="p-2">
            <label for="news_{{$n->id}}" class="inline-flex relative items-center cursor-pointer">
                <input type="checkbox" id="news_{{$n->id}}" name="news_{{$n->id}}" value="yes" class="sr-only peer" onclick="subscribe(this, '{{$n->id}}')"{{ $subscriber && $subscriber->subscripted($n->id) ? ' checked' : '' }}>
